/* creates record company database and sets it in use */
CREATE DATABASE record_company;
USE record_company;

/* creates a table called bands with an id and band name
...
/* creates a table called albums with an id and release year
links to the bands table by id*/
CREATE TABLE albums (
  id INT NOT NULL AUTO_INCREMENT,
  name VARCHAR(255) NOT NULL,
  release_year INT,
...
/* creates a table called songs with an id, name, and song length
links to the albums table by id */
CREATE TABLE songs (
...
/* inserts bands, albums, and songs */
...
/* selects bands by name as band name from bands tables. */
SELECT bands.name AS 'Band Name' From bands;

/* selects all albums info where release_year
is not null, by release_year in ascending order
but only 1 at most. */
SELECT * FROM albums
...
/* query only effective if all bands have unique names */
/* selects bands by name as band name from bands table joined
with albums table by id of primary and foreign key. */
SELECT bands.name AS 'Band Name' FROM bands
JOIN albums ON bands.id = albums.band_id;

/* selects bands by name as band name from bands table joined
with albums table by id of primary and foreign key then grouped by
bands.name if they have a count greater than 0 in the albums.id*/
SELECT bands.name AS 'Band Name' FROM bands
JOIN albums ON bands.id = albums.band_id
GROUP BY bands.name
...
/* selects bands by name as band name from bands table joined
with albums table by id of primary and foreign key then grouped by
bands.name if they have a count equalling 0 in the albums.id*/
SELECT
...
/* selects albums name as name, albums release year as release year,
and the sum of the songs as duration from albums then join songs by
primary key and foreign key of albums. Then ordered by duration is 
descending with a limit of one  */
SELECT 
	albums.name AS 'Name',
...
/* selects albums name as name, albums release year as release year,
and the sum of the songs as duration from albums then join songs by
primary key and foreign key of albums. Then ordered by duration is descending */
SELECT 
	albums.name AS 'Name',
	albums.release_year AS 'Release Year', 
	SUM(songs.length) AS 'Duration'
...
/* finds null release years from albums table */
SELECT * FROM albums
WHERE release_year IS NULL;

/* updates the albums table by setting year to 1986 for the album with id 4*/
UPDATE albums 
SET release_year = 1986
WHERE id = 4;

/* inserts a new band into the bands table
then selects all from the bands table */
INSERT INTO bands (name)
VALUES ('Get Scared');
SELECT * FROM bands;

/* inserts a new album into the albums table
then selects all from the albums table */
INSERT INTO albums (name, release_year, band_id)
VALUES ('Best Kind Of Mess', 2010, 8);
SELECT * FROM albums;

/* deletes the added album and band above from
the albums and bands table by id */
DELETE FROM albums
WHERE albums.id = 19;
SELECT * FROM albums;
DELETE FROM bands
...
/* gets the avg song length from the songs table */
SELECT AVG(songs.length) AS 'Average Song Duration' FROM songs;

/* selects album names, album release year, and the max 
song length on each album joins them into a table by the id key
then grouped by id */
SELECT 
...
/* selects band names and counts of songs with correspoding
id from the bands table then joins albums by id and songs by id.
Then groups then by the albums band id */
SELECT
	bands.name AS 'Band',
